...
export default function useSQS(){
  const {credentials,currentQueue,getFromQueue,deleteFromQueue,refreshVisibility}=useContext(UserContext)
  ...
  function getMessages(){
      ...
      console.log(`Index ${index}`)
        ...
        const bodies=messages.Messages.map(entity=>{
          const timer =  setInterval(()=>{refreshVisibility({ // ChangeMessageVisibilityRequest
          ...
          body.ack= async ()=>{
            try{
              await deleteFromQueue({
                ...
                clearInterval(timer)
                console.log(`Ack Succeeded for location ${body.id} with receipthandle ${entity.ReceiptHandle}`)
              }catch{
                console.log(`Ack Failed for location ${body.id} with receipthandle ${entity.ReceiptHandle}`)
              }
            };
          return body})
  ...
  return {buffer,index,next: buffer.length>(index+1)?next:undefined ,prev: index?prev:undefined, inject}
}