...
import { isMissingRow, withRowReinstated, withRowRestored, withoutRow } from '../utils/optimisticCache';
...
interface OptimisticModel<T> {
  create: (input: T) => Promise<{ data: T | null }>;
  update: (input: T) => Promise<{ data: T | null }>;
  delete: (input: T) => Promise<{ data: T | null }>;
  onCreate: (options?: SubscriptionOptions<T>) => Observable<T>;
  onUpdate: (options?: SubscriptionOptions<T>) => Observable<T>;
  onDelete: (options?: SubscriptionOptions<T>) => Observable<T>;
}
...
>(
  ...
  subscriptionFilter?: SubscriptionOptions<T>,
  ...
  const queryKey = useMemo(() => JSON.parse(queryHash) as [ModelKey, SubscriptionOptions<T> | undefined], [queryHash]);
  ...
  // The generated client exposes a union of model methods when indexed by a
  // generic key. Adapt that union once to this hook's row/selection contract.
  const model = client.models[modelKey] as unknown as OptimisticModel<T>;

  // Use the compositeKey function from options if provided, otherwise use id
  const getKey = useCallback((item: T): string => {
    if (compositeKeyRef.current) return compositeKeyRef.current(item);
    if ('id' in item && typeof item.id === 'string') return item.id;
    throw new Error('Rows without an id require a compositeKey resolver');
  }, []);
  ...
  */
  ...
  const createMutation = useMutation({
    ...
    onMutate: async (newItem) => {
      const keyedItem = newItem as T & { id?: string };
      keyedItem.id ||= crypto.randomUUID();
  ...
  return {
    ...
    meta: queryResult,
    create: (item: T) => {
      const keyedItem = item as T & { id?: string };
      keyedItem.id ||= crypto.randomUUID();
