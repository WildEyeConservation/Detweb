...
import { limitedClient } from '../api/limitedClient';
...
async function flushBatch(imageKey: string, batch: PendingTile[]) {
  try {
    const { data, errors } = await (limitedClient).queries.generateTile({
      imageKey,
...
export async function getTileBlob(path: string): Promise<Blob> {
  ...
  const attemptFetch = async (getUrlArgs: { path: string }): Promise<Blob> => {
