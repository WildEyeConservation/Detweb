...
const SubsampleModal: React.FC<SubsampleModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    for (const imageSet of selectedImageSets) {
      ...
      //Subsample images
      const subsampledImages = allImages.filter(
        (_, index) => index % subsampleInterval === 0
      );
      const newImageSetId = crypto.randomUUID();
      const promises = subsampledImages.map((image) =>
        ...
        <Form>
          ...
          <Form.Group className='mb-3'>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                setSubsampleInterval(Math.max(2, parseInt(e.target.value)))
              }
              min={2}
            />
