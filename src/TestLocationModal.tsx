...
export default function TestLocationModal({
  show,
  onClose,
  locationId,
  annotationSetId,
  ...
  useEffect(() => {
    async function getPresets() {
      ...
      const activePresets = await fetchAllPaginatedResults(
      ...
      ogActivePresets.current = activePresets.map((p) => ({
        id: p.testPresetId,
      }));
      ...
      setPresets(
        allPresets.map((p) => ({
          id: p.id,
          name: p.name,
          active: activePresets.some((ap) => ap.testPresetId === p.id),
        }))
      );
      setSelectedPresets(
        allPresets
  ...
  async function handleSubmit() {
    ...
    const annotations = rawAnnotations.filter((a) =>
      selectedPresetCategories.some((spc) => spc.categoryId === a.categoryId)
    );
