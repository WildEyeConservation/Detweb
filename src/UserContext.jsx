...
import { CreateQueueCommand, GetQueueAttributesCommand,GetQueueUrlCommand,ReceiveMessageCommand,SendMessageCommand,DeleteMessageCommand, SQSClient,ChangeMessageVisibilityCommand} from '@aws-sdk/client-sqs';
...
export default function User({ loggedInUser, children }) {
  ...
  async function removeUserFromGroup(config){
    ...
    useEffect(()=>{
      if (user){
        const projects=projectMemberships?.filter(pm=>pm.userId==user.id).map(pm=>pm.projectId)
    ...
    useEffect(()=>{
      if (user){
        setCurrentPM(projectMemberships?.find(pm=>pm.userId==user.id && pm.projectId==user.currentProjectId))
      }
    },[projectMemberships,user]) 
