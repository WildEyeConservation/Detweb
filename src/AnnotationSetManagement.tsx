...
export default function AnnotationSetManagement() {
  ...
  const {
    annotationSetsHook: { data: annotationSets, delete: deleteAnnotationSet },
    ...
    async function getTasks() {
      const result = await Promise.all(
        annotationSets.map((annotationSet) => {
          return fetchAllPaginatedResults(
            client.models.TasksOnAnnotationSet.list,
            {
              filter: { annotationSetId: { eq: annotationSet.id } },
              selectionSet: ['locationSet.name'] as const,
            }
          );
        })
      );
      ...
      if (!isCancelled) {
        ...
        setTasks(
          result.map((tasks) =>
            tasks
              .map((task) => ({ name: task.locationSet?.name || '' }))
  ...
  const tableData = annotationSets
