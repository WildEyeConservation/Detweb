...
function AddGpsData({show,handleClose}) {
  const {currentProject,gqlSend,gqlGetMany} = useContext(UserContext)
  const [selectedImageSets,setImageSets]=useState(undefined)
  ...
  async function handleSubmit(){       
    ...
    for (const selectedSet of selectedImageSets){
      setTotalSteps(0)
      var images= await gqlGetMany(getImagesInSet,{name:selectedSet},setStepsCompleted) 
      ...
      Papa.parse(file, {
        step: function(row,parser) {
          if (row.data.length>1){
            ...
            if (row.data.length<6 && row.data.length>1){ // Rob Wooding's GPS format as per mail on 1/12/2023
              format=1
            ...
            const [gpsStatus,timestamp,,lat,,lon,,alt,...rest]=row.data
            if (Number(gpsStatus)){
              const data = {timestamp:Number(timestamp)+(3600*2),lat:Number(lat),lon:Number(lon),alt:Number(alt)}
              while (data.timestamp>targetTime){
                if (prevData){
                  ...
                  const altitude_msl = prevData.alt*(1-pos)+data.alt*pos
                  const key=images[imageIndex].image.key
                  console.log('update image.')  
                  gqlSend(updateImage,{input:{key,latitude,longitude,altitude_msl}}).then(()=>setStepsCompleted(s=>s+1))
                  imageIndex+=1
                  ...
                  while(data.timestamp>targetTime)
                  console.log('images caught up to starting point of gps TrackEvent.')
                }
              }
              prevData=data
            }
        ...
        complete: function() {
          ...
          if (format==1){
            for (const {image:{key}} of images){
              try {
                const [lat,lon,height,transect]=lookup[key.split('/').pop()]  
                imageSets.add(transect)
                gqlSend(updateImage,{input:{key,latitude:parseFloat(lat),longitude:parseFloat(lon),altitude_agl:parseFloat(height)}})
                    .then(gqlSend(createImageSetMembershipMinimal,{input:{imageKey:key,imageSetName:`${selectedSet}_t${transect}`}}))
                    .then(()=>setStepsCompleted(sc=>sc+1))
                } catch (error) {
            ...
            for (const transect of imageSets){
              gqlSend(createImageSet,{input:{name:`${selectedSet}_t${transect}`,projectName:currentProject}})
            }
          }
          console.log("All done!");
        }
...
function handleChange(event) {
  ...
  return <Modal show={show} onHide={handleClose }>
    ...
    <Form>
      ...
      <ImageSetDropdown selectedSets={selectedImageSets} setImageSets={setImageSets}/>
    ...
    <Modal.Footer>
    <Button variant="primary" onClick={handleSubmit} disabled={!file || !selectedImageSets}>
        Submit 
      </Button>
