...
export function useUpdateProgress({taskId, indeterminateTaskName, determinateTaskName, stepName}){
        ...
        if (totalSteps || stepsCompleted){
            setProgress((progress)=>{ 
                ...
                if (totalSteps){
                    ...
                    progress[taskId]={value: stepsCompleted/totalSteps*100,
                        detail:<p>{determinateTaskName}<br/>
                        {`Done with ${stepsCompleted}/${totalSteps} ${stepName}`}<br/>
                        {`Elapsed time: ${humanizeDuration(millis,{units:["d","h","m","s"],round:true,largest:2})}`}<br/>
                        {`Estimated remaining time: ${humanizeDuration(remaining,{units:["d","h","m","s"],round:true,largest:1})}`}<br/>
                        </p>}
                }else{
                    progress[taskId]={detail:<p>{indeterminateTaskName}<br/>{`Loaded ${stepsCompleted} ${stepName} so far`}</p>}
                }
                // If the task has been completed we'll let the progress indicator hang around for 2 more seconds before removing it.
                if (stepsCompleted==totalSteps){
                    setTimeout(()=>setProgress(progress=>{progress={...progress}; delete progress[taskId];return progress}),2000)
                }
                return progress
            })}}
