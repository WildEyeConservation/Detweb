...
function getBounds(input: [number, number][]): [[number, number], [number, number]] {
  return input.reduce(
    (x, y) => [
      ...
      [Math.max(x[1][0], y[0]), Math.max(x[1][1], y[1])],
    ],
    [
      [Infinity, Infinity],
      [-Infinity, -Infinity],
...
const TransectSelector: React.FC<TransectSelectorProps> = (props) => {
  const map = useMap();

  useEffect(() => {
    map.fitBounds(getBounds(props.coords));
  }, [props.coords, map]);

  useEffect(() => {
    map.setView(props.coords[props.index], map.getZoom());
  }, [props.coords, map, props.index]);
