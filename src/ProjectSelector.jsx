...
function ProjectSelector() {    
   const {user,setCurrentProject,projects,currentProject} = useContext(UserContext)
   const {createProject} = useProjects()
   const {createProjectMembership} = useProjectMemberships()

   useEffect(()=>{
    if (projects?.length==0){
      alert("You have not been added to any projects yet. You won't be able to participate until an admin adds you to one of their projects and assigns you to a work queue. ")
    }
  ...
  return <>{(user?.isAdmin || projects?.length>1) && <Form> 
      <Form.Select 
        onChange={(e)=>{if (e.target.value=="new"){onNewProject().then(value=>setCurrentProject(value))} else setCurrentProject(e.target.value)}} 
