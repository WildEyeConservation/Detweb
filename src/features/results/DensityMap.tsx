...
import { useDialogRoute } from '../../shared/routing/useDialogRoute';
import { dialogIds } from '../../shared/routing/dialogValues';
import DialogNotice from '../../shared/routing/DialogNotice';
const MapViewerRoute = lazy(() => import('../images/MapViewerRoute'));
import {
...
import { useInfoTagData } from '../info-tags/useInfoTags';
import { formatInfoTagsForDisplay } from '../info-tags/infoTags';
...
export default function DensityMap({
  ...
  const annotationsFC = useMemo<FeatureCollection>(() => {
    ...
    for (const q of annotationQueries) {
      if (!q.data) continue;
      for (const a of q.data.annotations) {
        if (
          primaryOnly &&
          !(
            a.id === a.objectId &&
  ...
  const exclusionsFC = useMemo<FeatureCollection>(() => {
    ...
    for (const q of surveyQueries) {
      if (!q.data) continue;
      for (const e of q.data.exclusions) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addSource(SRC_IMAGES, { type: 'geojson', data: EMPTY_FC });
      ...
      map.on('click', LYR_POINTS, (e) => {
        ...
        pointPopup(
          (f.geometry as Point).coordinates.slice() as [number, number],
          name,
