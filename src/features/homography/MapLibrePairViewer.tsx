...
import { MapLibreImageViewer, type MapLibreMenuItem } from '../../shared/imaging/MapLibreImageViewer';
...
export function MapLibrePairViewer({
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      if (!cancelled) {
        setSourceKeys([keys[0], keys[1]]);
      }
    });
    return () => { cancelled = true; };
  }, [firstImageId, secondImageId, imagesRef]);
    ...
    Promise.all(
      imagesRef.current.map(async (img) => {
        ...
        const allNeighbourIds = [
          ...fwd.data.map((n) => n.image2Id),
          ...bwd.data.map((n) => n.image1Id),
        ];
      ...
      if (!cancelled) setNeighbours([result[0], result[1]]);
    });
    return () => { cancelled = true; };
  }, [firstImageId, secondImageId, imagesRef]);
