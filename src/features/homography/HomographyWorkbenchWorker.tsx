import { flatToPoints, resolveNeighbourDirection } from './homographyData';
...
import { countSuggestedPointsKept, homographyWorkflowTask, finalizeHomographyTask } from './homographyWorkflowStats';
import { type Point, MIN_HOMOGRAPHY_POINTS, solveHomography } from './homographyMath';
...
export function HomographyWorkbenchWorker({
  ...
  const handleSave = useCallback(
    async (H: Matrix) => {
      ...
      try {
        ...
        setSaveError(error instanceof Error ? getErrorMessage(error) : 'Failed to save homography');
        return false;
      } finally {
    ...
    [activeTime, imagesReadyAt, pair, queueId, onComplete, onSavePoints]
  );
  ...
  const handleSkip = useCallback(async () => {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? getErrorMessage(error) : 'Failed to skip pair');
    } finally {
      decisionInFlightRef.current = false;
      setIsSkipping(false);
    }
  ...
  return (
    ...
    <HomographyWorkbench
      images={[pair.primaryImage, pair.secondaryImage]}
      onSave={handleSave}
      onSkip={handleSkip}
