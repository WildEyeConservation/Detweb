...
import { recordWorkflowTask } from '../../shared/workflows/recordWorkflowTask';
import { useActiveTimeTracker } from '../../shared/hooks/useActiveTimeTracker';
import type { AnnotationType, ImageNeighbourType, ImageType } from '../../shared/api/schemaTypes';
import { useTransectData, type TransectData } from './hooks/useTransectData';
import { makePairKey, usePairWorkingState } from './hooks/usePairWorkingState';
...
import { buildChainSplitPlan, findDuplicateSameImageChainAnnotationIds, findSameImageAnnotationConflicts, type ChainSplitPlan } from './utils/chains';
import { findReunions, type ReunionCandidate } from './utils/reunionSearch';
import type { MatchCandidate, NeighbourPairWithMeta, PairCompletionState } from './types';
...
import ChangeCategoryModal from '../annotation/ChangeCategoryModal';
import type { CategoryType } from '../../shared/api/schemaTypes';
...
export function IndividualIdHarness({
  ...
  const handlePlaceNew = useCallback(
    async (
      ...
      try {
        await client.models.Annotation.create(dbInput);
      } catch (err) {
    ...
    [currentPair, categoryId, transect.data?.category?.annotationSetId, annotationSetId, project, patchTransectCache]
  );
    ...
    async (ids: string[], scope: 'single' | 'chain') => {
      ...
      try {
        const results = await Promise.all(
          ids.map((id) =>
            client.models.Annotation.delete({ id })
          )
        );
        await Promise.all(
          reRootUpdates.map((u) =>
  ...
  const executeSplitChain = useCallback(
    async (annotationId: string) => {
      ...
      try {
        await Promise.all(
          plan.updates.map((u) =>
            client.models.Annotation.update({ id: u.id, ...u.patch })
          )
        );
      } catch (err) {
  ...
  const setCategories = useMemo(
    () =>
      allCategories.filter(
        (c) => (c).annotationSetId === setIdForModal
      ),
  ...
  const commitActorLink = useCallback(
    async (
      ...
      void Promise.all([
        ...
        ...newRows.map((row) => {
          const { createdAt: _c, updatedAt: _u, ...dbInput } = row;
          return client.models.Annotation.create(dbInput);
        }),
      ]).catch((err) => {
    ...
    [transect.data?.category?.annotationSetId, transect.data?.imagesById, annotationSetId, project, localAnnotations, patchTransectCache]
  );
