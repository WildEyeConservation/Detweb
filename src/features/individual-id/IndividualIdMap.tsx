...
import { MapLocationOverlay, type LocationOverlayRow, type LocationSourceConfig } from './MapLocationOverlay';
...
export function IndividualIdMap({
  ...
  const startHideTimer = useEventCallback(() => {
    cancelHideTimer();
  ...
  const renderPopup = useEventCallback((key: string, mode: 'interactive' | 'passive') => {
  ...
  }, [map, markers, px2lngLat, lngLat2px, renderPopup, startHideTimer]);
  ...
  const imageName = (image).originalPath ?? image.id;
