...
import type { AnnotationType, CategoryType, ImageNeighbourType, ImageType } from '../../../shared/api/schemaTypes';
import { attachInfoTagsToAnnotations } from '../../info-tags/infoTags';
import { useCurrentProject } from '../../../shared/data/projectScope';
    ...
    queryFn: async () => {
      ...
      const extraImages = await Promise.all(
        extraImageIds.map(async (id) => {
          const resp = await (client.models.Image).get(
            { id },
            {
              selectionSet: [
                'id',
