...
import type { AnnotationType, CategoryType } from '../../shared/api/schemaTypes';
...
export function IndividualIdPairHarness({
  ...
  const handlePlaceNew = useCallback(
    ...
    [pair, annotationSetId, categoryId, project, patchPairCache]
  );
  ...
  const handleDelete = useCallback(
    async (annotationId: string) => {
      ...
      try {
        const result = await client.models.Annotation.delete({
          id: annotationId,
        });
        await Promise.all(
          reRootUpdates.map((u) =>
  ...
  const commitActorLink = useCallback(
    ...
    [annotationSetId, pairData.data?.imagesById, project, localAnnotations, patchPairCache]
  );
