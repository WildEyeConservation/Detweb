...
import { shouldIgnoreLaunchError } from '../../shared/workflows/launchErrors';
import { useCallback } from 'react';
import { uploadData } from 'aws-amplify/storage';
import { client } from '../../shared/api/appClient';
import type { LaunchQueueOptions, TiledLaunchRequest } from './LaunchTask';
...
export function useLaunchTask(
  ...
  const queryLocations = useEventCallback(async function queryLocations(
  ...
  const queryAnnotations = useEventCallback(async function queryAnnotations(
    annotationSetId: string,
    onProgress?: (message: string) => void
  ...
  const filterLocationsWithAnnotations = useEventCallback(function filterLocationsWithAnnotations(
    locations: LocationWithConfidence[],
  ...
  const launchTask = useCallback(
    ...
    [filterLocationsWithAnnotations, options.allowOutside, options.annotationSetId, options.batchSize, options.filterObserved, options.lowerLimit, options.projectId, options.skipLocationWithAnnotations, options.taskTag, options.upperLimit, options.zoom, queryAnnotations, queryLocations]
  );
