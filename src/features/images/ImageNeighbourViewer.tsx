...
import { makeProjection, createImageMap, addImageTiles, fitPxBounds } from '../../shared/imaging/imageTiles';
import { array2Matrix, makeTransform } from '../../shared/imaging/transforms';
...
export default function ImageNeighbourViewer() {
  ...
  const loadComparison = useCallback(
    async (idA: string, idB: string) => {
      ...
      try {
        ...
        setComparison(null);
        setError(
          getErrorMessage(err) ?? 'Unable to load the requested images or neighbours.'
        );
      } finally {
        setLoading(false);
      }
  ...
  const transformSummary: TransformSummary = useMemo(() => {
    ...
    if (forwardMatrix) {
      try {
        summary.toB = makeTransform(forwardMatrix);
        summary.toBSource = 'forward';
      } catch (err) {
        console.error('Failed to build forward transform', err);
      }
      try {
        summary.toA = makeTransform(inv(forwardMatrix));
    ...
    if (!summary.toB && reverseMatrix) {
      try {
        summary.toB = makeTransform(inv(reverseMatrix));
    ...
    if (!summary.toA && reverseMatrix) {
      try {
        summary.toA = makeTransform(reverseMatrix);
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      try {
        ...
        // A detached element cannot capture the pointer; keep normal dragging available.
      }
      event.preventDefault();
    };
