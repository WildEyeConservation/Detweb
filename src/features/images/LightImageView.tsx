...
import MapLibreLightViewer, { LightAnnotation } from '../annotation/annotator/MapLibreLightViewer';
...
export default function LightImageView({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Load image metadata
      const { data: image } = await (client).models.Image.get({ id: imageId }, { selectionSet: ['id', 'width', 'height'] as const });
      ...
      const filesResp = await (client).models.ImageFile.imagesByimageId({ imageId }, { selectionSet: ['id', 'type', 'key'] as const });
