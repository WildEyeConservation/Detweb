...
import { appRegion as region, backendOutputs as backend, client } from '../../shared/api/appClient';
...
export default function AwsServiceHealth() {
  ...
  const fetchEcsStatus = useCallback(async () => {
    ...
    try {
      ...
      setEcsState({
        clusters: [],
        isLoading: false,
        error: err instanceof Error ? getErrorMessage(err) : 'Unable to load ECS data.',
      });
    }
  }, []);
  ...
  const fetchQueueHealth = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? getErrorMessage(error) : 'Unable to refresh queues.';
  ...
  const handleDrainToDlq = useCallback(
    async (queueId: string) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? getErrorMessage(error) : 'Unable to drain queue.';
  ...
  const handleReplayFromDlq = useCallback(
    async (queueId: string) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? getErrorMessage(error) : 'Unable to replay DLQ.';
        setFeedback({ variant: 'danger', message });
      } finally {
        setActionState((prev) => ({ ...prev, [queueId]: undefined }));
      }
