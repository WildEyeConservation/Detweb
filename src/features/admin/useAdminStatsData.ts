    ...
    async function run() {
      ...
      try {
        ...
        for (const [orgId, projList] of projectsByOrg.entries()) {
          ...
          const orgSurveysTotal = projList.length;
          const orgSurveysThisMonth = projList.filter(
        ...
        if (!cancelled) setError(getErrorMessage(e) || 'Failed to load stats');
      } finally {
...
async function hasAnyClientLogBefore(
  ...
  try {
    const result = await (client.models.ClientLog.clientLogsByUserId)(
      { userId },
      {
