...
export default function ManageShares() {
  ...
  const onCreate = useCallback(async () => {
    ...
    try {
      ...
      setMessage(
        `Failed to create share: ${err instanceof Error ? getErrorMessage(err) : String(err)}`
      );
    } finally {
      setCreating(false);
    }
  ...
  const onAddReviewer = useCallback(
    async (shareId: string) => {
      ...
      try {
        ...
        setMessage(
          `Failed to add reviewer: ${err instanceof Error ? getErrorMessage(err) : String(err)}`
        );
      } finally {
        setBusyShare(null);
      }
  ...
  const onRemoveReviewer = useCallback(
    async (shareId: string) => {
      ...
      try {
        ...
        setMessage(
          `Failed to remove reviewer: ${err instanceof Error ? getErrorMessage(err) : String(err)}`
        );
      } finally {
        setBusyShare(null);
      }
  ...
  const onRevoke = useCallback(
    async (shareId: string) => {
      ...
      try {
        ...
        setMessage(
          `Failed to revoke: ${err instanceof Error ? getErrorMessage(err) : String(err)}`
        );
      } finally {
        setBusyShare(null);
      }
