import { getErrorDetails } from '../../../amplify/shared/errorMessage';
...
import { BASE_STYLE, EMPTY_FC, escapeHtml, type FeatureCollection } from './surveyMapStyle';
...
function errToString(err: unknown): string {
  ...
  const parts = [details.errorType, details.name, details.code, details.message].filter(
    Boolean
  );
...
async function runOp<T>(
  ...
  for (;;) {
    ...
    const errors = getErrorDetails(res).errors;
    if (errors && errors.length) {
      ...
      Object.assign(e, { graphQLErrors: errors });
      throw e;
    }
    return res;
  }
...
/** Retain each query's input and row types while retrying failed pages. */
function makeRetrying<Args extends unknown[], Result>(queryFn: (...args: Args) => Promise<Result>, signal?: AbortSignal) {
  return (...args: Args) => runOp(() => queryFn(...args), { signal });
}
...
export default function DeleteImages({ projectId, onClose }: { projectId: string; onClose: () => void }) {
  ...
  const fetchImages = useCallback(async (): Promise<ImageData[]> => {
    ...
    try {
      ...
      setError(
        `Failed to fetch images: ${err instanceof Error ? getErrorMessage(err) : String(err)
        }`
      );
      return [];
    } finally {
    ...
    async function loadShapefile() {
      ...
      try {
        const result = (await runOp(() =>
          (client.models.Shapefile.shapefilesByProjectId)({ projectId })
        ));
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addSource(SRC_SHAPEFILE, { type: 'geojson', data: EMPTY_FC });
      ...
      draw.on('finish', (id) => {
        try {
          ...
          const feature =
            snapshot.find((f) => f.id === id) ??
            snapshot.find((f) => f.geometry?.type === 'Polygon');
      ...
      await runPool(
        ...
        async (imageId) => {
          ...
          try {
            ...
            (leftNeighbours).forEach(pushNeighbour);
            (rightNeighbours).forEach(pushNeighbour);
      ...
      try {
        ...
        await runPool(
          ...
          async (agg) => {
            ...
            const del = async (fn: () => Promise<unknown>): Promise<boolean> => {
              try {
            ...
            for (const annotationId of agg.annotationIds) {
              if (
                await del(() =>
                  (client).models.Annotation.delete({ id: annotationId })
                )
              )
            ...
            for (const locationId of agg.locationIds) {
              if (
                await del(() =>
                  (client).models.Location.delete({ id: locationId })
                )
              )
            ...
            for (const membership of agg.membershipRecords) {
              if (
                await del(() =>
                  (client).models.ImageSetMembership.delete({
                    id: membership.id,
                  })
                )
              ) {
            ...
            for (const file of agg.fileRecords) {
              if (
                await del(() =>
                  (client).models.ImageFile.delete({ id: file.id })
                )
              )
            ...
            for (const neighbour of agg.neighbourPairs) {
              ...
              if (
                await del(() =>
                  (client).models.ImageNeighbour.delete({
            ...
            if (!imageFailed && !signal.aborted) {
              if (
                await del(() =>
                  (client).models.Image.delete({ id: agg.id })
                )
              )
        ...
        if (signal.aborted) {
          ...
          setError(
            `Delete operation failed: ${err instanceof Error ? getErrorMessage(err) : String(err)
            }`
          );
          setDeleteProgress(null);
        }
