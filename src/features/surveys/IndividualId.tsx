...
export default function IndividualId({
  ...
  }, [annotationSet.id, annotationSet.name, categories, project.id, project.organizationId, setIndividualIdLaunchHandler, user.userId]);
...
async function sendLaunchIndividualIdRequest(
  ...
  try {
    await (client).mutations.launchIndividualId(
