...
import { parseShapefileToLatLngs, saveShapefileForProject } from '../../shared/utils/shapefileUtils';
...
function polygonFeatureToLatLngs(feature: Feature): LatLng[] {
...
export default function EditShapeFile({
  ...
  useEffect(() => {
    ...
    const rebuildFromSnapshot = () => {
      ...
      const shp = polys.find((f) => f.properties.mode === MODE_SHAPEFILE);
      const exc = polys.filter(
        (f) => f.properties.mode === MODE_EXCLUSION
      );
      setPolygonCoords(shp ? polygonFeatureToLatLngs(shp) : null);
      setExclusionPolygons(exc.map((f) => polygonFeatureToLatLngs(f)));
    };
    ...
    try {
      ...
      await Promise.all(
        existingExclusions.map((ex) =>
          (client.models.ShapefileExclusions.delete)({ id: ex.id })
        )
      );
      await Promise.all(
        exclusionPolygons.map((poly) => {
          ...
          return (client.models.ShapefileExclusions.create)({
            projectId,
            coordinates: flatEx,
      ...
      await Promise.all(
        (strata).map((s) =>
          (client.models.Stratum.delete)({ id: s.id })
        )
      );
      await Promise.all(
        (jollyResults).map((r) =>
