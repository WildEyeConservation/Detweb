...
export default function HomographyLaunch({
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      try {
        ...
        if (!cancelled) {
          setLoadError(getErrorMessage(err) ?? 'Failed to load data');
          setLoading(false);
        }
      }
    }
  ...
  }, [annotationSet.id, annotationSet.name, categoryOptions.length, project.id, project.organizationId, selectedCategories.length, setHomographyLaunchHandler, user.userId]);
...
async function sendLaunchRequest(
  ...
  try {
    await (client).mutations.launchHomography({ request: requestPayload }, { retry: false });
  } catch (error) {
