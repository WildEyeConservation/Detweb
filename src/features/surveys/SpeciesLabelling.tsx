...
import { useLaunchTask } from '../tasks/useLaunchTask';
  ...
  const performLaunch = useCallback(
    async (
      ...
      if (modelGuidedRef.current) {
        ...
        const sets = (locationSetsRef.current || []).filter((ls) =>
          String(ls.name || '')
            .toLowerCase()
        ...
        await launchTaskRef.current({
          selectedTasks: sets.map((ls) => ls.id),
          onProgress,
          queueOptions: {
            name: 'Model Guided',
            hidden: false,
    ...
    [modelOptions, annotationSet.name, user.userId, project.name, project.id, project.organizationId]
  );
