...
type TransectImage = Omit<ImageType, 'timestamp' | 'latitude' | 'longitude'> & {
  timestamp: number; latitude: number; longitude: number; _imputed?: boolean;
};
type SegmentedImage = Omit<ImageType, 'transectId' | 'timestamp' | 'latitude' | 'longitude'> & { transectId: number; timestamp: number; latitude: number; longitude: number; _imputed?: boolean };
...
function toLatLngPairs(values: Array<number | null | undefined>): LatLng[] {
  ...
  for (let i = 0; i + 1 < values.length; i += 2) {
    ...
    if (isValidLatLng(lat, lng) && isFiniteNumber(lng)) {
      pairs.push([lat, lng]);
    }
  }
  return pairs;
...
function imputeLatLngForImages(
  imgs: ImageType[]
): { filled: TransectImage[]; imputedIds: Set<string> } {
  if (!imgs || imgs.length === 0) return { filled: [], imputedIds: new Set() };
  const filled: TransectImage[] = imgs.map(img => ({ ...img, timestamp: normalizeToMillis(img.timestamp), latitude: img.latitude ?? Number.NaN, longitude: img.longitude ?? Number.NaN }));
  ...
  const imputedIds = new Set<string>();
  for (let i = 0; i < n; i++) {
...
function normalizeToMillis(ts: string | number | null | undefined): number {
...
export default function DefineTransects({
  ...
  const [images, setImages] = useState<TransectImage[]>([]);
  ...
  const [imputedImageIds, setImputedImageIds] = useState<Set<string>>(new Set());
  ...
  const adjustedPositions = useMemo(() => {
    const byCoordKey: Record<string, SegmentedImage[]> = {};
  ...
  const clearStrata = () => {
    ...
    if (draw) {
      const ids = draw
        ...
        .map((f) => f.id).filter((id): id is string | number => id !== undefined);
      if (ids.length) {
        try {
  ...
  const handleSubmit = async () => {
    ...
    const existingStrata = await fetchAllPaginatedResults(
      ...
      { projectId, limit: 10000, selectionSet: ['id'] }
    );
    await Promise.all(
      existingStrata.map((st) =>
    ...
    for (const section of strataSections) {
      ...
      exclusionCoords.forEach((coords) => {
        ...
        const excCoordsLngLat = coords.map((pt) => {
          ...
          throw new Error('Invalid exclusion coordinate');
        });
        // ensure closure
        if (
          excCoordsLngLat[0][0] !==
      ...
      const createStratumRes = await (client.models.Stratum.create)({
        projectId,
        name: secName,
        area,
      ...
      if (!createStratumRes.data) throw new Error('Failed to create stratum');
      const newSid = createStratumRes.data.id;
    ...
    for (const tid of transectIds) {
      ...
      const createTransectRes = await (client.models.Transect.create)({
        projectId,
        stratumId: strId,
        group: organizationId,
      });
      if (!createTransectRes.data) throw new Error('Failed to create transect');
      const trId = createTransectRes.data.id;
    ...
    await Promise.all(
      segmentedImages.map(async (img) => {
        ...
        if (trId) {
          const update: Parameters<typeof client.models.Image.update>[0] = {
            id: img.id,
            transectId: trId,
          };
          ...
          await (client.models.Image.update)(update);
  ...
  useEffect(() => {
    async function loadExistingData() {
      ...
      if (existingStrata.length > 0) {
        ...
        const imgsWithTxRaw = rawImgsWithTx;
  ...
  useEffect(() => {
    async function loadShapefile() {
      const result = (await (
        client.models.Shapefile.shapefilesByProjectId
      )({ projectId }));
      const data = result.data as Array<{ coordinates: (number | null)[] }>;
  ...
  useEffect(() => {
    ...
    if (partsLoading === null && segmentedImages.length === 0) {
      ...
      const uniqueIds: number[] = Array.from(
        new Set(mergedSegImgs.map((img) => img.transectId))
  ...
  const splitPolygon = (
    ...
    const uniquePtsMap: Record<string, GeoJSONFeature<Point>> = {};
    intersections.features.forEach((f) => {
      const c = f.geometry.coordinates as [number, number];
      uniquePtsMap[uniqKey(c)] = f;
    });
    ...
    const withLoc = uniquePts
      .map((pt) => {
        ...
        return { pt, location: (onLine.properties).location as number };
      })
    ...
    const aIdx = (aOnRing.properties).index as number;
    const bIdx = (bOnRing.properties).index as number;
    ...
    const aT = ((aOnRing.properties)?.t as number | undefined) ?? 0;
    const bT = ((bOnRing.properties)?.t as number | undefined) ?? 1;
  ...
  const boundaryFC = useMemo<FeatureCollection>(() => {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addSource(SRC_STRATA, { type: 'geojson', data: EMPTY_FC });
      map.addSource(SRC_BOUNDARY, { type: 'geojson', data: EMPTY_FC });
      map.addSource(SRC_EXCLUSIONS, { type: 'geojson', data: EMPTY_FC });
      ...
      map.on('click', LYR_POINTS, (e) => {
        ...
        if (!f) return;
        const imageId = String(f.id ?? (f.properties)?.id ?? '');
      ...
      map.on('click', LYR_STRATA_FILL, (e) => {
        ...
        const node = document.createElement('div');
        node.style.color = '#000';
        node.innerHTML = `<div><strong>Stratum:</strong> ${(f.properties)?.id ?? ''
          }</div>`;
        showPopup(e.lngLat, node);
      });
