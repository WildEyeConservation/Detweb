import { getPolygonFeatures } from '../../shared/utils/shapefileUtils';
...
import { parseShapefileToLatLngs } from '../../shared/utils/shapefileUtils';
...
const GPSSubset: React.FC<GPSSubsetProps> = ({
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addSource(SRC_EXISTING, { type: 'geojson', data: EMPTY_FC });
      ...
      draw.on('finish', () => {
        try {
          ...
          setPolygonCount(
            snapshot.filter((f) => f.geometry?.type === 'Polygon').length
          );
        } catch {
          /* ignore */
        }
  ...
  useEffect(() => {
    ...
    const src = mapRef.current?.getSource(SRC_SHAPEFILE) as
      ...
      | undefined;
    if (!src) return;
    src.setData((shapefileFC ?? EMPTY_FC));
  ...
  const handleFilterPoints = useCallback(
    (exclude: boolean) => {
      const draw = drawRef.current;
      if (!draw) return;
      let polygons: Feature<Polygon | MultiPolygon>[] = [];
      try {
        polygons = draw
          .getSnapshot()
          .filter((f) => f.geometry?.type === 'Polygon')
          .map(f => ({ ...f, geometry: f.geometry as Polygon }));
      } catch {
        polygons = [];
      }
  ...
  const handleShapefileSelected = useCallback(
    async (file: File) => {
      try {
        ...
        const allowedPolygons = getPolygonFeatures(geojson);
        ...
        currentPointsRef.current.forEach((point) => {
          const inside = allowedPolygons.some((feature) =>
