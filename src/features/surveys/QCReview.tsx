...
export default function QCReview({
  ...
  }, [annotationSet.id, annotationSet.name, categories, project.id, project.name, project.organizationId, setQCLaunchHandler, user.userId]);
  ...
  do {
    ...
    for (const item of data || []) {
      allItems.push({
        ...
        reviewCatId: (item).reviewCatId ?? null,
...
async function sendLaunchQCReviewRequest(
  ...
  try {
    await (client).mutations.launchQCReview({
