...
import { backendOutputs as backend, client } from '../../shared/api/appClient';
import { uploadOrchestrator } from './core/UploadOrchestrator';
import { saveDirectoryHandle } from './core/dirHandles';
import type { UploadBackend } from './core/types';
import { downloadFailedImagesCsv, failedImagesCsvName } from './failedImagesCsv';
import { orientationCorrectionFor, orientationGroupForDimensions, type CameraOrientationRotations, type ImageOrientationGroup } from '../../shared/types/Orientation';
import exifr from 'exifr';
import { DateTime } from 'luxon';
import { fetchAllPaginatedResults } from '../../shared/api/pagination';
import Papa from 'papaparse';
import GPSSubset from '../surveys/GPSSubset';
import { parseGPX } from '@we-gold/gpxjs';
import FileInput from '../../shared/components/FileInput';
import FolderStructure from '../surveys/FolderStructure';
import ImageMaskEditor from '../images/ImageMaskEditor';
import Select from 'react-select';
import localforage from 'localforage';
import CameraOverlap from '../surveys/CameraOverlap';
import CameraSpecification from '../surveys/CameraSpecification';
...
import { buildGpsIndex, bracketTimestamp, interpolateAt, minMaxOf } from '../surveys/new-survey/gpsIndex';
import OrientationReview, { OrientationCameraGroup } from '../surveys/new-survey/OrientationReview';
import StepIndicator from '../surveys/new-survey/StepIndicator';
...
// Core functionality shared between modal and form versions
  ...
  async function getExifmeta(file: File) {
    ...
    let lat = tags?.latitude ?? tags?.GPSLatitude;
    let lng = tags?.longitude ?? tags?.GPSLongitude;
  ...
  function parseUtmString(
    ...
    try {
      ...
      // Invalid UTM coordinates are rejected by the caller when this returns null.
    }
    return null;
  }
  ...
  async function getTimestampOnlyExif(file: File) {
    ...
    let imageWidth =
      (tags)?.ImageWidth ?? (tags)?.ExifImageWidth ?? 0;
    let imageHeight =
      (tags)?.ImageHeight ?? (tags)?.ExifImageHeight ?? 0;
...
export function FileUploadCore({
  ...
  const convertAltitude = useCallback((altitude: number): number => {
    if (!altitudeInMeters) {
      ...
      try {
        ...
        const restResults = await mapWithLimit(
          ...
          async (file) => {
            ...
            if (
              exif.gpsData &&
              (exif.gpsData).alt !== undefined &&
              (exif.gpsData).lat !== undefined &&
              (exif.gpsData).lng !== undefined
            ) {
              const lat = Number((exif.gpsData).lat);
              const lng = Number((exif.gpsData).lng);
              // Only set GPS data if coordinates are valid and in range
  ...
  }, [imageFiles, name, newProject, loadingExistingImages, existingImagePathSet, convertAltitude]);
  ...
  const getGpsForFile = useCallback(
    ...
    [associateByTimestamp, csvData, csvIndex, exifData, convertAltitude]
  );
  ...
  const csvImportOptions = useLatestRef({ columnMapping, commonTimezone, convertAltitude, exifData, filteredImageFiles, useUtm, utmColumn });
  useEffect(() => {
    if (!file || !mappingConfirmed) return;
    // A confirmed import is one operation; map filtering must not restart it.
    const { columnMapping, commonTimezone, convertAltitude, exifData, filteredImageFiles, useUtm, utmColumn } = csvImportOptions.current;
    async function transformFile(file: File) {
      ...
      Papa.parse<Record<string, string>>(file, {
        ...
        complete: function (results) {
          ...
          const hasFilepath = results.data.some((row) => row['FilePath']);
          const hasUtm = useUtm && results.data.some((row) => row['UTM']);
  ...
  }, [file, mappingConfirmed, csvImportOptions]);
  ...
  }, [timeRanges, fullCsvData, associateByTimestamp, applyTimeFilter]);
...
export default function FilesUploadComponent({
  ...
  const handleModalSubmit = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? getErrorMessage(error) : 'Unable to prepare the upload. Please try again.');
    } finally {
