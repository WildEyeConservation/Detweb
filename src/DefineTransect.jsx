...
import {LayersControl, MapContainer, TileLayer, Polyline, LayerGroup, useMap} from 'react-leaflet'
...
function getBounds(input){
  return input.reduce((x,y) => y[0] ? [[Math.max(x[0][0],y[0]),Math.max(x[0][1],y[1])],[Math.min(x[1][0],y[0]),Math.min(x[1][1],y[1])]] : x,[[-Infinity,-Infinity],[Infinity,Infinity]])
}


function TransectSelector({imageLocations,selectedRange} ) {
  const positions=imageLocations.filter(loc=>loc.image.latitude).map(loc=>[loc.image.latitude,loc.image.longitude])
  const map=useMap();
  ...
  const startIndex=imageLocations.findIndex(iml=>iml.image.timestamp>=selectedRange[0])
  const endIndex=imageLocations.findIndex(iml=>iml.image.timestamp>=selectedRange[1])
  //useEffect(()=>{map.setView(props.coords[props.index],map.getZoom())},[props.coords,map,props.index]);
  return <>{ready && <LayerGroup>
    <Polyline pathOptions={{color:'blue'}} positions={positions.slice(0,startIndex+1)}/>
    <Polyline pathOptions={{color:'red'}} positions={positions.slice(startIndex,endIndex+1)}/>
    <Polyline pathOptions={{color:'lightblue'}} positions={positions.slice(endIndex)}/>  
    </LayerGroup>}</>
}
...
`
  ...
  useEffect(()=>{
    if (currentProject){
        gqlGetMany(getImageSetsInProject,{name:currentProject}).then(sets=>setImageSets(sets))
    }
  },[currentProject])
  ...
  useEffect(()=>{
    if (selectedSet){
      const res1 = gqlGetMany(getImagesInSet,{name:selectedSet}).then(iml=>iml.sort((a,b)=>a.image.timestamp-b.image.timestamp))
      res1.then(iml=>setImageLocations(iml))
      res1.then(iml=>setSliderRange([iml[0].image.timestamp, iml[iml.length-1].image.timestamp]))
    }
  }
,[selectedSet])
  ...
  const createSubset=(imageLocations,start,end,name)=>{
    const promises = []
    const startIndex=imageLocations.findIndex(iml=>iml.image.timestamp>=start)
    const endIndex=imageLocations.findIndex(iml=>iml.image.timestamp>=end)  
    for (const {image} of imageLocations.slice(startIndex,endIndex+1)){
      promises.push(gqlSend(createImageSetMembershipMinimal,{input:{imageKey: image.key,imageSetName:name}}))
    }
    ...
    return <Container fluid>
      ...
      {imageSets?.map( q => <option key={q.name} value={q.name}>{q.name}</option>)} 
        ...
        <Row className="min-vh-100">
          <Col>
        <MapContainer bounds={[[80,180],[-80,180]]}>
          ...
          <LayersControl.Overlay checked name="Current sortie">
            <TransectSelector imageLocations={imageLocations} selectedRange={[selectStart ? selectStart : sliderValue, sliderValue]}/>
          ...
          <RangeSlider className="transectSlider"
                        value={sliderValue}
                        onChange={(e) => {
