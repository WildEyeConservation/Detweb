...
function LaunchTask({ show, handleClose }: LaunchTaskProps) {
  ...
  const [locationSet, setLocationSet] = useState<string | null>(null);
  ...
  const { sendToQueue, gqlSend, gqlGetMany } = userContext;
  ...
  async function handleSubmit() {
    ...
    const promises = [];
    setTotalSteps(0);
    const locations = await gqlGetMany(
      getLocationsInSet,
      { id: locationSet },
      setStepsCompleted,
    );
    ...
    for (const location of locations) {
      if (filterObserved) {
        const response = await gqlSend(checkForObservations, { locationId: location.id });
        const { items } = (response as ObservationsResponse).data.observationsByLocationId;
        if (items.length > 0) {
      ...
      promises.push(
        ...
        }).then(() => setStepsCompleted((fc: any) => fc + 1)),
      );
    }
    ...
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Footer>
        <Button
          ...
          disabled={!locationSet || !annotationSet || !url}
        >
          Submit
        </Button>
