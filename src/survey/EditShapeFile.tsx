...
export default function EditShapeFile({
  ...
  const onDeleted = useCallback((_e?: any) => {
    if (drawMode !== 'shapefile') return;
      ...
      <Form.Group className='mt-3'>
        <Form.Group className='d-flex flex-column mb-2'>
          ...
          <span className='text-muted mt-1' style={{ fontSize: '14px' }}>
            Use the polygon tool with the selected mode to draw or edit. Exclusion
            zones will be deducted from the total area when computing results.
          </span>
        </Form.Group>
        <div style={{ height: '600px', width: '100%', position: 'relative' }}>
          ...
          >
            ...
            <FeatureGroup ref={exclusionFeatureGroupRef}>
              ...
              {drawMode === 'exclusions' && exclusionFeatureGroupRef.current ? (
                <EditControl
                  position='topright'
                  draw={{
                    rectangle: false,
                  ...
                  edit={{ featureGroup: exclusionFeatureGroupRef.current } as any }
                />
              ) : null}
            </FeatureGroup>
          </MapContainer>
