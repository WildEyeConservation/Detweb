...
export function withPreloading2(WrappedComponent){
    let WithPreloading=function({historyN,preloadN,...rest}){
        ...
        //   }
        // });
        const {buffer,index,next,prev,inject}= useSQS(3)     
        ...
        if (subset?.length){
            return <>
            {subset.map((entry,i)=>
            <div key={entry.message_id} style={{ visibility: i===(index-subsetStart) ?'visible':'hidden',
            position:'absolute',justifyContent: 'center',display: i===(index-subsetStart)?'contents':'flex',width:'80%' }}>
                {/*  */}
            <WrappedComponent {...entry} 
            ...
            <Modal size="xl" centered show={errorprops} backdrop="static" onHide={()=>setErrorprops(false)}>
                <Modal.Header closeButton>
                <Modal.Title>Looks like you missed some animals. Perhaps it is time to take a break?</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                <Row>
                <Col/>
                ...
                {errorprops &&
                <BaseImage
                    {...errorprops} containerwidth='100%' containerheight = '800px'>
                    <Location {...errorprops} />
                </BaseImage>}
                </Col>
                <Col/>
                ...
                <Modal.Footer>
                <Button variant="primary" onClick={()=>{setErrorprops(false)}}>
                    OK
                </Button>
                </Modal.Footer>
            </Modal>          
