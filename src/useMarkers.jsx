...
export function useMarkers(imageId,setId){
  const {annotations,createAnnotation,deleteAnnotation,updateAnnotation}=useAnnotations(imageId,setId);
  ...
  useEffect(()=>{
    setMarkers(annotations?.map(annotation=>
      <Marker
        ...
        */
        key={annotation.y + annotation.id + annotation.categoryId + annotation.x} 
        eventHandlers={{
          dragend:(e)=>{
      ...
      >
      <Popup>Users : {annotation?.user?.name}<br/>Created : {annotation?.createdAt}</Popup>
    </Marker>
    ))
