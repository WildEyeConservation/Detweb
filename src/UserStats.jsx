...
export default function UserStats() {
  ...
  async function getObservations() {
    ...
    do {
      ({
        data: {
          userProjectMembershipsByProjectId: { items, nextToken },
        },
    ...
    do {
      ({
        data: {
          listObservations: { items, nextToken },
        },
      } = await gqlClient.graphql(
        graphqlOperation(listObservations, { nextToken }),
      ));
      for (const item of items) {
        observationsByUser[item.owner].push({
    ...
    for (const user of users) {
      let prev = 0,
        ...
        tests = 0,
        passes = 0;
      for (const obs of observationsByUser[user].sort(
        ...
        prev = obs.createdAt;
        if (numBySet[obs.annotationSetId] == 1) {
          //test case detected
          tests += 1;
          const {
            data: {
              annotationsByAnnotationSetId: { items },
            },
          } = await gqlClient.graphql(
      ...
      const userStats = {
        user,
        totalTime,
        totalJobs: observationsByUser[user].length,
        tests,
        passes,
      };
      console.log(
