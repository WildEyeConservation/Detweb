...
class leafletS3Layer extends L.GridLayer { 
  ...
  createTile(coords, done) {
    ...
    if (!(key in cachedUrlPromises)){
      // Get a blob URL
      cachedUrlPromises[key]=this.getObject({ Bucket: backend.outputsBucketOut, Key: key })
      .then(response => {
      ...
      cachedUrlPromises[key].then(()=>setTimeout(()=>delete cachedUrlPromises[key],1800000)) 
    }
    cachedUrlPromises[key]
      .then(url=>{
        tile.src = url;
        setTimeout(function(){done(null, tile)},1)
      })
    return tile
  }
}
