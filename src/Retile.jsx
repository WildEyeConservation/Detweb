...
function Retile({show,handleClose}) {
  ...
  async function handleSubmit(){    
    ...
    for (const imageSet of selectedSets){
      do{
        ({data:{imageSetMembershipsByImageSetName:{items,nextToken}}}=await gqlSend(imageSetMembershipsByImageSetName,{imageSetName:imageSet,nextToken}))
        allItems = allItems.concat(items);
      } while (nextToken)
      //  allItems=allItems.filter(({imageKey})=>imageKey.startsWith("SangoDay1A/1122"))
      allItems.map(({imageKey})=>{
        const params = {
          FunctionName: "arn:aws:lambda:af-south-1:275736403632:function:detweb-stack-develop-tileImage42302E57-exz2oBIP9AFq",
          Payload: JSON.stringify({Records:[
            { s3:
              {bucket: 
                { name:backend.imagesBucketOut }, 
  ...
  return <Modal show={show} onHide={handleClose }>
    ...
    <Form>
      ...
      <ImageSetDropdown setImageSets={setImageSets} selectedSets={selectedSets}/> 
      </Form.Group>
