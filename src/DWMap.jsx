...
class DWMap extends L.map{
                ...
                for (let row of csv.data.slice(1)) {
                  ...
                  for (let i = 0; i < row[countIndex]; i++) {
                    let marker = L.marker(coord, {
                      draggable: false, icon: L.divIcon(
                        {
                          className: "my-custom-pin", iconAnchor: [0, 24], labelAnchor: [-6, 0], popupAnchor: [0, -36],
  ...
  constructor(mapId) {
    ...
    this.layercontrol.addBaseLayer(L.gridLayer.googleMutant({
      type: 'satellite'
    }),"Google Satellite");
    this.layercontrol.addBaseLayer(L.gridLayer.googleMutant({
      type: 'roadmap'
    ...
    let mapbox=L.tileLayer('https://api.mapbox.com/v4/mapbox.satellite/{z}/{x}/{y}.jpg90?' +
