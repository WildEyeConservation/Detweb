...
export default function MoveObservations({
  ...
  const [existingAnnotationSetId, setExistingAnnotationSetId] =
    useState<string>('');
  ...
  const handleMove = async () => {
    if (
      ...
      selectedAnnotationSets.includes(existingAnnotationSetId)
    ) {
    ...
    try {
      ...
      const filteredObservations =
        selectedLocationSets.length > 0
          ? observations.filter((observation) =>
              selectedLocationSets.includes(observation.location?.setId || '')
            )
          : observations;
      ...
      if (
        !confirm(
          `Are you sure you want to move ${
            ...
            selectedLocationSets.length > 0 ? ` (filtered)` : ''
          }?`
        )
      ) {
        return;
      ...
      const previousAnnotationSetName = (
        annotationSets.find((set) => set.id === selectedAnnotationSets[0])
          ?.name || 'set'
      ).replace(/ /g, '_');
    ...
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Footer>
        <Button
          ...
          disabled={
            (!selectedUserId && !observationTime) ||
            selectedAnnotationSets.length === 0 ||
            (existingAnnotationSetId === '' && !moveToNewAnnotationSet)
          }
        >
          Move Observations
        </Button>
