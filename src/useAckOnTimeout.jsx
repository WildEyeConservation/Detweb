...
export default function useAckOnTimeout({next,visible,ack}){
    ...
    const onNext=useCallback(()=>{
        ...
        if (next){
            ...
            alert("You seem to have reached the end of the current Task queue. Thanks for your contribution. Please check back later to see if more work has been loaded. Alternatively you can close the notification, but leave the window open and you will get notified via another popup when new work has been loaded.")
            setDone(true)
        }
    },[next,ack])
