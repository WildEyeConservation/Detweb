...
export default function useGqlCached(
  {
    queryKey,
    listItem,
    createItem,
...
export const useImages = () => {
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      ...
      onCreate: (subConfig) =>
        gqlSend(subs.onCreateImage).then((x) => x.subscribe(subConfig)),
      onDelete: (subConfig) =>
        gqlSend(subs.onDeleteImage).then((x) => x.subscribe(subConfig)),
      onUpdate: (subConfig) =>
        gqlSend(subs.onUpdateImage).then((x) => x.subscribe(subConfig)),
    });
  return {
    images: query.data,
...
export const useImageSets = (currentProject) => {
  ...
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      queryKey: ["imageSets"],
      listItem: async () => await gqlSend(queries.listImageSets, filter),
      createItem: async (input) =>
      ...
      onCreate: (subConfig) =>
        gqlSend(subs.onCreateImageSet).then((x) => x.subscribe(subConfig)),
      onDelete: (subConfig) =>
        gqlSend(subs.onDeleteImageSet).then((x) => x.subscribe(subConfig)),
      onUpdate: (subConfig) =>
        gqlSend(subs.onUpdateImageSet).then((x) => x.subscribe(subConfig)),
    });
  return {
...
export const useUsers = () => {
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      queryKey: ["users"],
      listItem: async () => await gqlSend(queries.listUsers, {}),
      createItem: async (input) =>
      ...
      onCreate: (subConfig) =>
        gqlSend(subs.onCreateUser).then((x) => x.subscribe(subConfig)),
      onDelete: (subConfig) =>
        gqlSend(subs.onDeleteUser).then((x) => x.subscribe(subConfig)),
      onUpdate: (subConfig) =>
        gqlSend(subs.onUpdateUser).then((x) => x.subscribe(subConfig)),
    });
  return {
    users: query.data,
    createUser: (newUser) => {
...
export const useCategory = (currentProject) => {
  ...
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      ...
      createItem: async (input) =>
        await gqlSend(mutations.createCategory, {
          input: { ...input, projectName: currentProject },
        }),
      updateItem: async ({ id, name, color, shortcutKey }) =>
        await gqlSend(mutations.updateCategory, {
          input: { id, name, color, shortcutKey, projectName: currentProject },
        }),
      deleteItem: async ({ id }) =>
        await gqlSend(mutations.deleteCategory, { input: { id } }),
      onCreate: (subconfig) =>
        gqlSend(subs.onCreateCategory).then((x) => x.subscribe(subconfig)),
      onDelete: (subconfig) =>
        gqlSend(subs.onDeleteCategory).then((x) => x.subscribe(subconfig)),
      onUpdate: (subconfig) =>
        gqlSend(subs.onUpdateCategory).then((x) => x.subscribe(subconfig)),
    });
  return {
    ...
    useGqlCached({
      ...
      listItem: async () => {
        ...
        while (nextToken) {
          ...
          const {
            data: { annotationsByImageKey: la },
          } = await gqlSend(listAnnotations, { nextToken, setId, imageKey }); //filter:{imageId:{eq:imageId},setId:{eq:setId}}
      ...
      createItem: async ({
        ...
        await gqlSend(createAnnotationMinimal, {
          input: {
            id,
            x,
            y,
      ...
      updateItem: async ({
        ...
        await gqlSend(updateAnnotationMinimal, {
          input: {
            id,
            x,
            y,
    ...
    useGqlCached(
      {
        ...
        listItem: async () =>
          await gqlSend(queuesByProjectId, { projectId: project }),
        ...
        updateItem: async ({ id, name, url }) =>
          await gqlSend(mutations.updateQueue, {
            input: { name, url, id, projectId: projectName },
          }),
        deleteItem: async ({ id }) =>
...
export const useAnnotationSets = (project) => {
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      ...
      listItem: async () =>
        await gqlSend(queries.annotationSetsByProjectName, {
          projectName: project,
        }),
      createItem: async ({ name, id }) =>
        await gqlSend(mutations.createAnnotationSet, {
          input: { id, name, projectName: project },
        }),
...
export const useLocations = (setId) => {
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      ...
      onCreate: (subconfig) =>
        gqlSend(createSub, { setId }).then((x) => x.subscribe(subconfig)),
      onDelete: (subconfig) =>
    ...
    useGqlCached(
      {
        ...
        onCreate: (subconfig) =>
          gqlSend(subs.onCreateProject).then((x) => x.subscribe(subconfig)),
        onDelete: (subconfig) =>
          gqlSend(subs.onDeleteProject).then((x) => x.subscribe(subconfig)),
        onUpdate: (subconfig) =>
          gqlSend(subs.onUpdateProject).then((x) => x.subscribe(subconfig)),
      },
      (a, b) => a.name === b.name,
    );
  return {
...
export const useProjectMemberships = () => {
  const { query, createMutation, deleteMutation, updateMutation } =
    useGqlCached({
      ...
      listItem: async () =>
        await gqlSend(queries.listUserProjectMemberships, {}),
      createItem: async (input) =>
      ...
      onCreate: (subconfig) =>
        gqlSend(subs.onCreateUserProjectMembership).then((x) =>
          x.subscribe(subconfig),
        ),
      onDelete: (subconfig) =>
        gqlSend(subs.onDeleteUserProjectMembership).then((x) =>
          x.subscribe(subconfig),
        ),
      onUpdate: (subconfig) =>
        gqlSend(subs.onUpdateUserProjectMembership).then((x) =>
          x.subscribe(subconfig),
        ),
    });
  return {
