...
const FitBoundsToImages: React.FC<{ imageSetsData: ImageSetData[] }> = ({
  imageSetsData,
}) => {
  const map = useMap();
  ...
  useEffect(() => {
    if (imageSetsData.length > 0) {
      const bounds = L.latLngBounds(
        imageSetsData.flatMap((imageSet) =>
          imageSet.images.map(
            (image) => [image.latitude, image.longitude] as [number, number]
          )
        )
      );
...
const SpatiotemporalSubset: React.FC<CreateSubsetModalProps> = ({
  show,
  handleClose,
  selectedImageSets,
  project,
  ...
  const fetchFilenames = async (imageId: string) => {
    const { data: images } = await client.models.ImageFile.imagesByimageId({
      imageId,
    });
    ...
    if (path) {
      const { url } = await getUrl({
        path: 'slippymaps/' + path + '/0/0/0.png',
  ...
  useEffect(() => {
    const fetchImagesData = async () => {
      ...
      const fetchedImageSets = await Promise.all(
        selectedImageSets.map(async (selectedSetId) => {
  ...
  const handleDeleted = useCallback((e: any) => {
    ...
    layers.eachLayer((layer: L.Layer) => {
      if (layer instanceof L.Polygon) {
        const subsetId = (layer as any).subsetId;
        setSubsets((prevSubsets) =>
          prevSubsets.filter((subset) => subset.id !== subsetId)
        );
      }
    });
  }, []);
  ...
  const createNewImageSet = useCallback(
    async (name: string, imageIds: string[]) => {
  ...
  const handleCreateSubsets = useCallback(() => {
    subsets.forEach((subset) => {
      ...
      if (
        JSON.stringify(polygonCoords[0]) !==
        JSON.stringify(polygonCoords[polygonCoords.length - 1])
      ) {
          ...
          {loading && (
            ...
            >
              ...
              <span style={{ marginLeft: '10px' }}>
                Please be patient while geodata is loaded
              </span>
            </div>
          )}
          <MapContainer
