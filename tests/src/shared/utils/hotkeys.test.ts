...
import { canonicalHotkeyFromParsed, findShortcutMatch, formatShortcutKey, normalizeShortcutKey } from '../../../../src/shared/utils/hotkeys';
...
test('stored shortcuts normalise to the same form the matcher produces', () => {
  ...
  assert.equal(normalizeShortcutKey('./utils'), 'period');
...
test('shortcuts recorded as physical key names match the character pressed', () => {
  ...
  assert.equal(
    findShortcutMatch(keyEvent('./utils', 'Period'), categories, (c) => c.shortcutKey)
      ?.id,
    'dot'
  );
});
...
test('shortcuts render readably', () => {
  ...
  assert.equal(formatShortcutKey('period'), './utils');
